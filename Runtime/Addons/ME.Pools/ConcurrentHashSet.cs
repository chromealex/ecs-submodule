...
namespace ME.ECS {
    ...
    namespace ConcurrentCollections {
        ...
        public class ConcurrentHashSet<T> : IReadOnlyCollection<T>, ICollection<T> {
            ...
            /// </summary>
            public ConcurrentHashSet() : this(ConcurrentHashSet<T>.DefaultConcurrencyLevel, ConcurrentHashSet<T>.DefaultCapacity, true, null) { }
            ...
            public ConcurrentHashSet(IEqualityComparer<T>? comparer) : this(ConcurrentHashSet<T>.DefaultConcurrencyLevel, ConcurrentHashSet<T>.DefaultCapacity, true, comparer) { }
            ...
            /// </exception>
            public ConcurrentHashSet(int concurrencyLevel, IEnumerable<T> collection, IEqualityComparer<T>? comparer) : this(
                concurrencyLevel, ConcurrentHashSet<T>.DefaultCapacity, false, comparer) {
                if (collection == null) {
                    throw new ArgumentNullException(nameof(collection));
                }
            ...
            public void Clear() {
                ...
                try {
                    ...
                    var newTables = new Tables(new Node[ConcurrentHashSet<T>.DefaultCapacity], tables.Locks, new int[tables.CountPerLock.Length]);
            ...
            public bool TryGetValue(T equalValue, out T actualValue) {
                ...
                var bucketNo = ConcurrentHashSet<T>.GetBucket(hashcode, tables.Buckets.Length);
            ...
            public struct Enumerator : IEnumerator<T> {
                ...
                public Enumerator(ConcurrentHashSet<T> set) {
                    ...
                    this._i = -1;
                    this._state = ConcurrentHashSet<T>.Enumerator.StateUninitialized;
                }
                ...
                public bool MoveNext() {
                    switch (this._state) {
                        case ConcurrentHashSet<T>.Enumerator.StateUninitialized:
                            this._buckets = this._set._tables.Buckets;
                            this._i = -1;
                            goto case ConcurrentHashSet<T>.Enumerator.StateOuterloop;

                        case ConcurrentHashSet<T>.Enumerator.StateOuterloop:
                            ...
                            if ((uint)i < (uint)buckets!.Length) {
                                ...
                                this._state = ConcurrentHashSet<T>.Enumerator.StateInnerLoop;
                                goto case ConcurrentHashSet<T>.Enumerator.StateInnerLoop;
                            }

                            goto default;

                        case ConcurrentHashSet<T>.Enumerator.StateInnerLoop:
                            var node = this._node;
                            if (node != null) {
                        ...
                        default:
                            this._state = ConcurrentHashSet<T>.Enumerator.StateDone;
                            return false;
                    }
                }
            ...
            private void GrowTable(Tables tables) {
                ...
                try {
                    ...
                    // Add more locks
                    if (this._growLockArray && tables.Locks.Length < ConcurrentHashSet<T>.MaxLockNumber) {
                    ...
                    for (var i = 0; i < tables.Buckets.Length; i++) {
                        ...
                        while (current != null) {
                            ...
                            ConcurrentHashSet<T>.GetBucketAndLockNo(current.Hashcode, out var newBucketNo, out var newLockNo, newBuckets.Length, newLocks.Length);
