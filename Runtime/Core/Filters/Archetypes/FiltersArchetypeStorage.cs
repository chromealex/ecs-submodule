...
namespace ME.ECS.FiltersArchetype {
    ...
    public struct FiltersArchetypeStorage : IStorage {
        ...
        public struct Archetype {
            ...
            internal static int CreateAdd(ref MemoryAllocator allocator, ref FiltersArchetypeStorage storage, int node, in List<int> componentIds, in Dictionary<int, Info> components, int componentId) {
                ...
                var arch = new Archetype() {
                    ...
                    components = new Dictionary<int, Info>(ref allocator, components.Count + 1),
                };
            ...
            [MethodImplAttribute(MethodImplOptions.AggressiveInlining)]
            #endif
            internal static int CreateRemove(ref MemoryAllocator allocator, ref FiltersArchetypeStorage storage, int node, in List<int> componentIds, in Dictionary<int, Info> components, int componentId) {
        ...
        public unsafe void Alloc(ref MemoryAllocator allocator, int count, ref EntitiesGroup group, Unity.Collections.Allocator unityAllocator, bool copyMode) {
            ...
            Unity.Collections.LowLevel.Unsafe.UnsafeUtility.MemCpy(array.GetUnsafePtr(), this.cache.GetUnsafePtr(ref allocator), size * count);
