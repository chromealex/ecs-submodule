...
namespace ME.ECS {
    ...
    public static partial class VecMath {
        ...
        public static tfloat Angle(float2 lhs, float2 rhs) {
            var num = math.sqrt(math.lengthsq(lhs) * math.lengthsq(rhs));
            return num < tfloat.Epsilon ? 0 : math.degrees(math.acos(math.clamp(math.dot(lhs, rhs) / num, -1, 1)));
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static tfloat Angle(float3 lhs, float3 rhs) {
            var num = math.sqrt(math.lengthsq(lhs) * math.lengthsq(rhs));
            return num < tfloat.Epsilon ? 0 : math.degrees(math.acos(math.clamp(math.dot(lhs, rhs) / num, -1, 1)));
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static tfloat SignedAngle(float2 lhs, float2 rhs) {
            return VecMath.Angle(lhs, rhs) * math.sign(lhs.x * rhs.y - lhs.y * rhs.x);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static tfloat SignedAngle(float3 lhs, float3 rhs, float3 axis) {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static float3 Lerp(float3 lhs, float3 rhs, tfloat t) {
            return math.lerp(lhs, rhs, t);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static float2 Lerp(float2 lhs, float2 rhs, tfloat t) {
            return math.lerp(lhs, rhs, t);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static float3 Slerp(float3 lhs, float3 rhs, tfloat t) {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static float3 RotateTowards(float3 lhs, float3 rhs, tfloat angleMove, tfloat magnitudeMove) {
            ...
            if (lhsMag > VecMath.VECTOR3_EPSILON && rhsMag > VecMath.VECTOR3_EPSILON) {
                ...
                // direction is almost the same
                if (dot > 1 - VecMath.VECTOR3_EPSILON) {
                    return VecMath.MoveTowards(lhs, rhs, magnitudeMove);
                }
                // directions are almost opposite
                else if (dot < -1 + VecMath.VECTOR3_EPSILON) {
                    var axis = VecMath.OrthoNormalVectorFast(lhsNorm);
                    var m = fpmatrix3x3.zero;
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static float3 MoveTowards(float3 lhs, float3 rhs, tfloat clampedDistance) {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static tfloat ClampedMove(tfloat lhs, tfloat rhs, tfloat clampedDelta) {
            var delta = rhs - lhs;
            if (delta > 0) {
