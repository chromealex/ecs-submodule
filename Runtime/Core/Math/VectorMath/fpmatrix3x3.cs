namespace ME.ECS {
    ...
    public static class fpmatrix3x3Ext {
        ...
        public static ref fp Get(this ref fpmatrix3x3 m, int row, int column) {
    ...
    public struct fpmatrix3x3 {
        ...
        public fp3 MultiplyVector3(fp3 v) {
            fp3 res;
        ...
        public void MultiplyVector3(fp3 v, fp3 output) {
        ...
        public fp3 MultiplyVector3Transpose(fp3 v) {
            fp3 res;
        ...
        public void SetOrthoNormalBasis(fp3 inX, fp3 inY, fp3 inZ) {
            this.Get(0, 0) = inX[0];
            this.Get(0, 1) = inY[0];
            this.Get(0, 2) = inZ[0];
            this.Get(1, 0) = inX[1];
        ...
        public void SetOrthoNormalBasisInverse(fp3 inX, fp3 inY, fp3 inZ) {
            this.Get(0, 0) = inX[0];
            this.Get(1, 0) = inY[0];
            this.Get(2, 0) = inZ[0];
            this.Get(0, 1) = inX[1];
        ...
        public void SetScale(fp3 inScale) {
        ...
        private static void swap(ref fp lhs, ref fp rhs) {
        ...
        public bool Invert() {
            ...
            if (fpmath.abs(fDet) > VecMath.VECTOR3_EPSILON) {
                kInverse /= fDet;
                this = kInverse;
                return true;
            } else {
        ...
        public static fpmatrix3x3 operator *(fpmatrix3x3 m, fpmatrix3x3 inM) {
            int i;
            for (i = 0; i < 3; i++) {
                fp3 v = new Vector3(m.Get(i, 0), m.Get(i, 1), m.Get(i, 2));
        ...
        public void SetAxisAngle(fp3 rotationAxis, fp radians) {
            this.GetRotMatrixNormVec(ref this, rotationAxis, radians);
        }

        public void SetFromToRotation(fp3 from, fp3 to) {
        ...
        public void MakePositive(fp3 euler) {
            fp negativeFlip = -0.0001F;
            fp positiveFlip = fpmath.PI * 2.0F - 0.0001F;

            if (euler.x < negativeFlip) {
                euler.x += 2.0 * fpmath.PI;
            } else if (euler.x > positiveFlip) {
                euler.x -= 2.0 * fpmath.PI;
            }

            if (euler.y < negativeFlip) {
                euler.y += 2.0 * fpmath.PI;
            } else if (euler.y > positiveFlip) {
                euler.y -= 2.0 * fpmath.PI;
            }

            if (euler.z < negativeFlip) {
                euler.z += 2.0 * fpmath.PI;
            } else if (euler.z > positiveFlip) {
                euler.z -= 2.0 * fpmath.PI;
            }
        }

        public void SanitizeEuler(fp3 euler) {
            this.MakePositive(euler);
        }

        public void EulerToMatrix(fp3 v, fpmatrix3x3 matrix) {
            fp cx = fpmath.cos(v.x);
            fp sx = fpmath.sin(v.x);
            fp cy = fpmath.cos(v.y);
            fp sy = fpmath.sin(v.y);
            fp cz = fpmath.cos(v.z);
            fp sz = fpmath.sin(v.z);
        ...
        public bool MatrixToEuler(fpmatrix3x3 matrix, fp3 v) {
            ...
            if (matrix.Get(1, 2) < 0.999F) // some fudge for imprecision
            {
                if (matrix.Get(1, 2) > -0.999F) // some fudge for imprecision
                {
                    v.x = fpmath.asin(-matrix.Get(1, 2));
                    v.y = fpmath.atan2(matrix.Get(0, 2), matrix.Get(2, 2));
                    v.z = fpmath.atan2(matrix.Get(1, 0), matrix.Get(1, 1));
                    this.SanitizeEuler(v);
                    return true;
                } else {
                    // WARNING.  Not unique.  YA - ZA = atan2(r01,r00)
                    v.x = fpmath.PI * 0.5F;
                    v.y = fpmath.atan2(matrix.Get(0, 1), matrix.Get(0, 0));
                    v.z = 0.0F;
                    this.SanitizeEuler(v);
         ...
         */
        private void fromToRotation(ref fp3 from, ref fp3 to, ref fpmatrix3x3 mtx) {
            fp3 v;
            fp e, h;
            v = fpmath.cross(from, to);
            e = fpmath.dot(from, to);
            if (e > 1.0) /* "from" almost or equal to "to"-vector? */ {
                /* return identity */
                mtx.Get(0, 0) = 1.0;
                mtx.Get(0, 1) = 0.0;
                mtx.Get(0, 2) = 0.0;
                ...
                mtx.Get(2, 1) = 0.0;
                mtx.Get(2, 2) = 1.0;
            } else if (e < -1.0) /* "from" almost or equal to negated "to"? */ {
                fp3 up, left = Vector3.zero;
                fp invlen;
                fp fxx, fyy, fzz, fxy, fxz, fyz;
                fp uxx, uyy, uzz, uxy, uxz, uyz;
                fp lxx, lyy, lzz, lxy, lxz, lyz;
                /* left=CROSS(from, (1,0,0)) */
                left[0] = 0.0f;
                left[1] = from[2];
                left[2] = -from[1];
                if (fpmath.dot(left, left) < 0.00001f) /* was left=CROSS(from,(1,0,0)) a good choice? */ {
                ...
                /* normalize "left" */
                invlen = 1.0f / fpmath.sqrt(fpmath.dot(left, left));
                left[0] *= invlen;
                left[1] *= invlen;
                left[2] *= invlen;
                up = fpmath.cross(left, from);
                ...
                fp hvx, hvz, hvxy, hvxz, hvyz;
                h = (1.0f - e) / fpmath.dot(v, v);
                hvx = h * v[0];
                hvz = h * v[2];
                hvxy = hvx * v[1];
                hvxz = hvx * v[2];
        ...
        // Right handed
        private bool LookRotationToMatrix(fp3 viewVec, fp3 upVec, fpmatrix3x3 m) {
            var z = viewVec;
            // compute u0
            fp mag = fpmath.length(z);
            if (mag < 0.00001f) {
                m.SetIdentity();
                return false;
            }
            ...
            var y = fpmath.cross(z, x);
            if (fpmath.lengthsq(y) == 1.0F) {
                return false;
            }
        ...
        private void GetRotMatrixNormVec(ref fpmatrix3x3 data, fp3 inVec, fp radians) {
            /* This function contributed by Erich Boleyn (erich@uruk.org) */
            /* This function used from the Mesa OpenGL code (matrix.c)  */
            fp s, c;
            fp vx, vy, vz, xx, yy, zz, xy, yz, zx, xs, ys, zs, one_c;
