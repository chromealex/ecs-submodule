...
namespace ME.ECS
{
    ...
    public partial struct fp : IEquatable<fp>, IComparable<fp>
    {
        ...
        public static double precision => new fp(1L);//0.00000000023283064365386962890625m;
        public static fp MaxValue => new fp(MAX_VALUE);
        public static fp MinValue => new fp(MIN_VALUE);
        ...
        public static fp operator /(fp x, fp y)
        {
            ...
            while (remainder != 0 && bitPos >= 0)
            {
                ...
                {
                    return ((xl ^ yl) & MIN_VALUE) == 0 ? fp.MaxValue : fp.MinValue;
                }
        ...
        {
            return x.m_rawValue == MIN_VALUE ? fp.MaxValue : new fp(-x.m_rawValue);
        }
        ...
        public static bool operator <=(fp x, float y)
        {
            return x.m_rawValue <= ((fp)y).m_rawValue;
        }

        public static bool operator <=(float x, fp y)
        {
            return ((fp)x).m_rawValue <= y.m_rawValue;
        }
        ...
        internal static fp Pow2(fp x)
        {
            ...
            if (x >= Log2Max)
            {
                return neg ? one / fp.MaxValue : fp.MaxValue;
            }
            if (x <= Log2Min)
            {
                return neg ? fp.MaxValue : zero;
            }
        ...
        {
            // Up to 10 decimal places
            return ((double)this).ToString("0.##########", formatProvider);
        }
    ...
    partial struct Fix64 
    {
        ...
        {");
                ...
                for (int i = 0; i < LUT_SIZE; ++i)
                {
                    ...
                    if (tan > (double)fp.MaxValue || tan < 0.0)
                    {
                        tan = (double)fp.MaxValue;
                    }
                    var rawValue = (((double)tan > (double)fp.MaxValue || tan < 0.0) ? fp.MaxValue : (fp)tan).m_rawValue;
                    writer.Write(string.Format("0x{0:X}L, ", rawValue));
                }
                writer.Write(
@"
