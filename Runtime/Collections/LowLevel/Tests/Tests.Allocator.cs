...
namespace ME.ECS.Collections.LowLevel.Unsafe.Tests {
    ...
    public class Tests_Allocator {
        ...
        private void TestProxy(AllocatorDebugProxy proxy, AllocatorDebugProxy proxy2) {
        ...
        public void AllocArray() {
            var amount = 1000;
            MemArrayAllocator<int> ptr1;
            MemArrayAllocator<int> ptr2;
            {
                var arr = ptr1 = new MemArrayAllocator<int>(ref this.allocator, amount);
            ...
            {
                var arr = ptr2 = new MemArrayAllocator<int>(ref this.allocator, amount);
                for (int i = 0; i < amount; ++i) {
                    arr[in this.allocator, i] = i;
                }
        ...
        public void AllocList() {
            var amount = 10000u;
            {
                ...
                for (uint i = 0u; i < amount; ++i) {
                    Assert.IsTrue(arr[in this.allocator, (int)i] == i);
                }
            }
        }
        ...
        public void Deserialize() {
            ...
            var proxy = new AllocatorDebugProxy(this.allocator);
            this.allocator.CheckConsistency();
            
            var reader = new StreamBufferReader(writer.ToArray());
            this.allocator = new MemoryAllocator(Unity.Collections.Allocator.Persistent);
            this.allocator.Deserialize(ref reader);
            var proxy2 = new AllocatorDebugProxy(this.allocator);
            this.allocator.CheckConsistency();
        ...
        public void CopyFrom() {
            ...
            var newAllocator = new MemoryAllocator(Unity.Collections.Allocator.Persistent);
            newAllocator.CopyFrom(this.allocator);
            newAllocator.CheckConsistency();
            this.TestProxy(new AllocatorDebugProxy(this.allocator), new AllocatorDebugProxy(newAllocator));
        ...
        public unsafe struct JobAlloc : Unity.Jobs.IJobParallelFor {
            ...
            public safe_ptr<MemoryAllocator> allocator;
