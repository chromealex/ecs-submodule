...
public class EnumMaskPropertyDrawer : PropertyDrawer
{
    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        if (foldoutOpen)
        {
            ...
            {
                if (EditorGUI.Toggle(new Rect(position.x, position.y + EditorGUIUtility.singleLineHeight * (2 + i), position.width, EditorGUIUtility.singleLineHeight), System.Enum.GetNames(fieldInfo.FieldType)[i], IsSet(i)))
                {
                    ToggleIndex(i, true);
                }
                else
